/**
 * Este archivo define la clase Accesos, que registra cuando un usuario reproduce una cancion.
 * Mantiene un historial (log) de reproducciones con ID de usuario, cancion y fecha/hora.
 */
...
/** Representa un registro de acceso o reproduccion de una cancion por un suscriptor. */
class Accesos {
    private:
        int _idSuscriptor;
        int _idCancion;
    ...
    public:
        ...
        // --- SETTERS ---
        /** Establece el ID del suscriptor que realizo el acceso. */
        void setIdSuscriptor(int id);
        /** Establece el ID de la cancion accedida. */
        ...
        bool Guardar();
        /** Lee un registro de acceso desde el archivo en la posicion especificada. */
        bool Leer(int pos);
        /** Obtiene la cantidad total de registros de acceso en el archivo. */
        int ObtenerCantidadRegistros();

        // No solemos "Modificar" o "Buscar por ID" un acceso porque es un log historico,
