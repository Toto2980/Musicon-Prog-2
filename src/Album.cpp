 ...
 */
bool Album::Guardar() {
    return ArchivoBinario<Album>(cfg::archivos::ALBUMES).agregar(*this);
}
...
bool Album::Leer(int pos) {
    return ArchivoBinario<Album>(cfg::archivos::ALBUMES).leer(pos, *this);
}
...
bool Album::Modificar(int pos) {
    return ArchivoBinario<Album>(cfg::archivos::ALBUMES).escribir(pos, *this);
}
...
int Album::ObtenerCantidadRegistros() {
    return ArchivoBinario<Album>(cfg::archivos::ALBUMES).contar();
}
...
int Album::BuscarIDPorTitulo(const char* titulo) {
    std::vector<Album> todos = ArchivoBinario<Album>(cfg::archivos::ALBUMES).leerTodos();
...
int Album::BuscarPosicionPorID(int id) {
    ...
    for (size_t pos = 0; pos < todos.size(); ++pos) {
        if (todos[pos].getIdAlbum() == id && todos[pos].getEstado()) {
