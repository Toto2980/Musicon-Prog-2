...
// Parsea una linea CSV respetando comillas dobles. Local a este archivo.
static vector<string> parsearCSV(string linea) {
...
void CancionManager::Modificar() {
    ...
    for(int i = 0; i < total; i++) {
        Canciones c = _archivoCanciones.Leer(i);
        if(Texto::igualesSinMayusculas(c.getNombre(), nombreBuscado) && c.getEstado()) {
            pos = i;
            break;
        }
    }
...
void CancionManager::RegistrarAcceso(int idSuscriptor) {
    ...
    for (int i = 0; i < total; i++) {
        Canciones c = _archivoCanciones.Leer(i);
        if (Texto::igualesSinMayusculas(c.getNombre(), nomC) && c.getEstado()) {
            idC = c.getIdCancion();
            break;
        }
    }
