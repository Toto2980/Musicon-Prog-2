...
*   - Eliminar: hace eliminacion LOGICA (estado=false), nunca borra del archivo.
...
* Primero pide el nombre de usuario y verifica que no este tomado (unicidad).
* Si el nombre ya existe, lo rechaza y vuelve a pedir uno nuevo.
* Luego genera un ID automatico y llama a nuevo.Cargar() para el resto de los datos.
*/
void SuscriptorManager::Agregar() {
	...
	// Bucle hasta conseguir un nombre de usuario unico
	while (true) {
	...
	// Genera ID unico y asigna el nombre ya validado
	int idAuto =
...
* Por ahora solo permite cambiar el email (dejar vacio para no modificar).
*/
void SuscriptorManager::Modificar() {
	...
	// Solo cambia si el usuario escribio algo
	if(strlen(email) > 0)
		s.setEmail(email);
...
/**
* Da de baja logicamente a un suscriptor (estado=false).
* Busca por nombre, muestra los datos y pide confirmacion antes de proceder.
...
void SuscriptorManager::Eliminar() {
	...
	if(op == 's' || op == 'S') {
		...
		// Eliminacion logica: marcamos estado=false y sobrescribimos el registro
		s.setEstado(false);
...
/**
* Listar: alias de ListarActivos para la interfaz del menu principal.
*/
...
void SuscriptorManager::ListarActivos() {
	...
	// Lee todos los registros de una sola vez (mas eficiente que un fread por registro)
	Suscriptor *lista =
...
* Mismo mecanismo que ListarActivos pero con la condicion invertida.
*/
...
* Util para el admin, que necesita ver el historial completo.
*/
